...
/workspace/SCTK/bin/sclite  -h ${workdir}$tmp_prefix.tmp.out.$hypothesisCTM ctm -r ${workdir}$tmp_prefix.tmp.stm stm -f 0 -o sgml sum rsum pra
/workspace/SCTK/bin/sclite  -h ${workdir}$tmp_prefix.tmp.out.$hypothesisCTM ctm -r ${workdir}$tmp_prefix.tmp.stm stm -f 0 -o dtl stdout |grep Error
